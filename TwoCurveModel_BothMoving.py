...
BBc0=[-(i*i/500+15)+10 for i in range(deg+1)]
BBd0=[-(i*i/500)+10 for i in range(deg+1)]
...
SYc=[sum(BBc[j]*Bezier(j,deg,X[i]) for j in range(len(BBc))) for i in range(len(X))]
SYd=[sum(BBd[j]*Bezier(j,deg,X[i]) for j in range(len(BBd))) for i in range(len(X))]

SYc0=[sum(BBc0[j]*Bezier(j,deg,X[i]) for j in range(len(BBc))) for i in range(len(X))]
...
Bc=[X[i]*R.x+SYc[i]*R.y for i in range(len(X))]
...
mirrors=[(Bd[i+1]-Bc[i]) for i in range(len(X)-1)]
...
S=R.y#+1*R.x
S/=sqrt(S&S)
concentrationPoint=-10.

#َ A نقطة التركيز
A=concentrationPoint*R.y#-0*R.x

#
Exp=[N[i]^(sqrt((A-Bc[i])&(A-Bc[i]))*S+A-Bc[i]) for i in range(1,len(X)-1)]
#

ControlPoints=BBc.copy()
...
InitialControlPoints=BBc0.copy()
InitialControlPoints.extend(BBd0)
res = minimize(rosen, np.array(InitialControlPoints), method='nelder-mead',
               options={'xatol': 1e-8, 'disp': True,"maxiter": 100000})

# تمثيل الحلول هندسيا
#نقاط التحكم 
BBcSol=res.x[range(len(BBc))]
BBdSol=res.x[range(len(BBc),len(BBc)+len(BBd))]
#تراتيب المنحنى
Ysolc=[sum(BBcSol[j]*Bezier(j,deg,X[i]) for j in range(len(BBcSol))) for i in range(len(X))]
